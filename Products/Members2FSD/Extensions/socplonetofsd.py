...
class memberConverter(object):
    ...
    def getOldMemberProperties(self, item):
        site = getSite()
      ...
      def __call__(self, member):
	   ...
	   catalog = getToolByName(site, 'portal_catalog')
        ...
        memberprops = self.getOldMemberProperties(items)
...
def convert():
    ...
    for memid in mt.listMemberIds():
        ...
        transaction.commit()
        try:
            memcontent = site.people[memid].objectIds()
            ref = site.people[memid].manage_cutObjects(memcontent)
            site._p_jar.sync()
